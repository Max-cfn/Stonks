...
**Pré-requis :** la PR Phase 2.1 (PR #2 vraisemblablement) doit être **mergée sur `main`** avant que tu commences le code. Vérifie en début de mission via `gh pr view 2 --repo Max-cfn/Stonks --json mergedAt,state` :
- Si `state: MERGED` → OK, continue
- Si `state: OPEN` → fais `gh pr checks 2` pour voir l'état CI. Si tout est vert, demande à l'humain de merger via `request_human_approval`. Si rouge, **n'attaque pas** Phase 2.2 — termine le finalize d'abord et loggue un échec explicatif.
- Si tu as accès au backend FastAPI/Postgres/Vault/JWT/AES-GCM de Phase 2.1, tu peux importer depuis `stonks_backend.*` normalement.
