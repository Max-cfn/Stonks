...
1. **Vérifier le process Next.js** :
   ```bash
   ps aux | grep "next dev" | grep -v grep
   cat /proc/$(pgrep -f "next dev" | head -1)/cmdline | tr '\0' ' '
   ```
   → Note si `--turbo` est présent. Si oui, passe à `next dev` sans
   Turbopack pour voir si ça résout.

2. **Vérifier next.config.ts** :
   - `file_read` sur `apps/web/next.config.ts`
   - Cherche `basePath`, `assetPrefix`, `headers()`, `reactStrictMode`
   - Vérifie que `output` n'est pas `export`

3. **Vérifier les composants layout racine** :
   - `apps/web/src/app/layout.tsx` et `[locale]/layout.tsx`
   - Cherche `useSearchParams()`, `usePathname()` sans `<Suspense>`
   - Cherche `typeof window`, `Date.now()` dans le JSX

4. **Activer les logs d'hydratation React** :
   - Dans `next.config.ts`, ajouter temporairement :
     ```ts
     reactStrictMode: true,
     ```
   - Relancer `apps/web` avec `NODE_ENV=development`
   - Surveiller la console pour des erreurs d'hydratation

5. **Test E2E avec Playwright** (si dispo) :
   - Naviguer vers `/fr/login`
   - Screenshot + console logs
   - Vérifier si `window.__NEXT_DATA__` est présent
   - Vérifier si `React.startTransition` / `hydrateRoot` est appelé

6. **Isoler le problème** :
   - Créer une page minimale (`/fr/test`) sans hooks, sans composants
     shadcn/ui — juste un `<button onClick={() => alert('works')}>`.
   - Si le bouton marche → le problème est dans un composant importé
   - Si le bouton ne marche pas → le problème est global (Next.js config,
     layout racine, ou pipeline de build)
